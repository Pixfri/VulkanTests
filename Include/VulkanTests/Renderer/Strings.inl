...
namespace VkTests {
	...
	std::string ToString(const VkPresentModeKHR presentMode) {
		switch (presentMode) {
...
#undef STR
	...
	std::string ToString(const VkSurfaceFormatKHR surfaceFormat) {
		std::string surfaceFormatString = ToString(surfaceFormat.format) + ", ";
	...
	std::string ToString(const VkVertexInputRate rate) {
		switch (rate) {
	...
	std::string ToString(SceneGraph::AlphaMode mode) {
		if (mode == SceneGraph::AlphaMode::Blend) {
			return "Blend";
		}
		else if (mode == SceneGraph::AlphaMode::Mask) {
			return "Mask";
		}
		else if (mode == SceneGraph::AlphaMode::Opaque) {
			return "Opaque";
		}
		return "Unknown";
	}
	...
	std::string ToString(const ShaderResourceType type) {
		switch (type) {
	...
	std::string ImageCompressionFlagsToString(VkImageCompressionFlagsEXT flags) {
		return 0 == flags
			       ? "VK_IMAGE_COMPRESSION_DEFAULT_EXT"
			       : ToString<VkImageCompressionFlagsEXT>(flags,
			                                              {
				                                              {
	...
	inline std::string ImageCompressionFixedRateFlagsToString(VkImageCompressionFixedRateFlagsEXT flags) {
		return 0 == flags
			       ...
			       : ToString<VkImageCompressionFixedRateFlagsEXT>(flags,
			                                                       {
				                                                       {
	...
	template <typename T>
	std::string ToString(uint32_t bitmask, const std::map<T, const char*> stringMap) {
