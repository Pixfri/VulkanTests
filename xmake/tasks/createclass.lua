...
on_run(function()
    ...
    local files = {
        { TargetPath = path.join("Include/VulkanTests", classPath) .. ".hpp", Template = headerTemplate },
        { TargetPath = path.join("Include/VulkanTests", classPath) .. ".inl", Template = inlineTemplate }
    }

    if not option.get("nocpp") then
        table.insert(files, { TargetPath = path.join("Source", "VulkanTests", classPath) .. ".cpp", Template = sourceTemplate })
    end
    ...
    local replacements = {
        ...
        HEADER_GUARD = "VK_TESTS_" .. classPath:gsub("[/\\]", "_"):upper() .. "_HPP"
    }
...
#include <VulkanTests/%CLASS_PATH%.inl>
...
#include <VulkanTests/%CLASS_PATH%.hpp>
